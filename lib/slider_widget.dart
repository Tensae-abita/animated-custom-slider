...
import 'package:sania/tagged_sequence_animation.dart';
...
class _SliderState extends State<Slider> with TickerProviderStateMixin {
  ...
  late TaggedSequenceAnimation sequenceAnimation;
  ...
  void initState() {
    ...
    _dragPosition =
        Offset(widget.ultimateWidth * 0.4, widget.ultimateHeight * 0.78);
    _dragPercentage = 2.5;
  ...
  void _initAnimation() {
    _sliderAnimation =
        Tween(begin: animBegin, end: animENd).animate(_animationController)
          ..addListener(() {
            setState(() {});
          });
  }
    ...
    sequenceAnimation = buildSequenceAnimation(
      ...
      items: [
        ...
        SequenceItem(
            ...
            to: const Duration(milliseconds: 2200)),
        SequenceItem(
            tag: 'angle',
            begin: 0.0,
            end: 0.0,
  ...
  void _UpdateDragPosion(Offset val) {
    ...
    setState(() {
      ...
      if (newDragPosition == widget.ultimateWidth * 0.22) {
        _dragPercentage = 0.0;
      } else if (newDragPosition > widget.ultimateWidth * 0.22 &&
          newDragPosition < widget.ultimateWidth * 0.29) {
        _dragPercentage = 0.5;
      } else if (newDragPosition > widget.ultimateWidth * 0.29 &&
          newDragPosition < widget.ultimateWidth * 0.36) {
        _dragPercentage = 1.0;
      } else if (newDragPosition > widget.ultimateWidth * 0.36 &&
          newDragPosition < widget.ultimateWidth * 0.43) {
        _dragPercentage = 1.5;
      } else if (newDragPosition > widget.ultimateWidth * 0.43 &&
          newDragPosition < widget.ultimateWidth * 0.50) {
        _dragPercentage = 2.0;
      } else if (newDragPosition > widget.ultimateWidth * 0.50 &&
          newDragPosition < widget.ultimateWidth * 0.57) {
        _dragPercentage = 2.5;
      } else if (newDragPosition > widget.ultimateWidth * 0.57 &&
          newDragPosition < widget.ultimateWidth * 0.64) {
        _dragPercentage = 3.0;
      } else if (newDragPosition > widget.ultimateWidth * 0.64 &&
          newDragPosition < widget.ultimateWidth * 0.71) {
        _dragPercentage = 3.5;
      } else if (newDragPosition > widget.ultimateWidth * 0.71 &&
          newDragPosition < widget.ultimateWidth * 0.79) {
        _dragPercentage = 4.0;
      } else if (newDragPosition > widget.ultimateWidth * 0.79 &&
          newDragPosition < widget.ultimateWidth * 0.86) {
        _dragPercentage = 4.5;
      } else if (newDragPosition > widget.ultimateWidth * 0.86 &&
          newDragPosition < widget.ultimateWidth * 0.90) {
        _dragPercentage = 5.0;
      }
    });
  }
  ...
  void _onDragUpdate(BuildContext context, DragUpdateDetails update) {
    ...
    setState(() {
      _dragPosition =
          Offset(offset.dx - widget.ultimateWidth * 0.155, offset.dy);
      getRotation();
      isDragging = true;
    });
    _getWidgetInfo();
...
// animation after end of drag
  ...
  void _onDragEnd(BuildContext context, DragEndDetails end) {
    ...
    _getWidgetInfo();
    emoji = false;
    emojiPlace = _dragPosition.dy -
        widget.ultimateHeight * 0.024 -
        (widgetPosition - widget.ultimateHeight * 0.75);
    ...
    if (doubletap) {
      ...
      setState(() {
        _dragPosition = Offset(
            _doubleTapDetails.localPosition.dx, widget.ultimateHeight * 0.78);
      });
...
// animation after end of drag
  ...
  getRotation() {
    double angle = 0;
    if (GetContainerBack) {
      if (_dragPosition.dy > widget.ultimateHeight * 0.81 ||
          _dragPosition.dy < widget.ultimateHeight * 0.76) {
        setState(() {
  ...
  void _onDragUpdateContainer(BuildContext context, DragUpdateDetails update) {
    ...
    _UpdateDragPosion(offset);
    setState(() {
      _dragPosition = Offset(offset.dx - widget.ultimateWidth * 0.155,
          widgetPosition + widget.ultimateHeight * 0.03);
      getRotation();
      isDragging = true;
    });
  }
  ...
  void _onDoubleTapContainer() {
    ...
    if (doubletap) {
      ...
      setState(() {
        _dragPosition = Offset(
            _doubleTapDetails.localPosition.dx - widget.ultimateWidth * 0.155,
            widget.ultimateHeight * 0.78);
      });
...
//play audio
  void PlayAudio() {
    ...
    if (_dragPercentage >= 4.0) {
      player
          .play(AssetSource('sounds/sound_for_rating_between_2.0_to_3.5.mp3'));
  ...
  _onlongPressEnd(LongPressEndDetails details) {
    ...
    if (doubletap == true) {
      ...
      setState(() {
        _dragPosition = Offset(
            details.localPosition.dx - widget.ultimateWidth * 0.155,
            widget.ultimateHeight * 0.78);
      });
      Future.delayed(Duration(milliseconds: 1500), () {
        // Do something
    ...
    return Stack(
      children: [
        Positioned(
            top: 0,
            child: Container(
                width: MediaQuery.of(context).size.width, child: Appbar())),
        Container(
              ...
              children: [
                Stack(
                  children: [
                    Container(
                      height:
                          widget.ultimateHeight - (widget.ultimateHeight * 0.1),
                    ),

                    Positioned(
                        left: !GetContainerBack
                            ? widget.ultimateWidth * 0.4 -
                                widget.ultimateWidth * 0.415
                            : _dragPosition.dx - widget.ultimateWidth * 0.415,
                        top: !GetContainerBack
                            ? widget.ultimateHeight * 0.78 -
                                widget.ultimateHeight * 0.72
                            : (_dragPosition.dy -
                                    (widgetPosition -
                                        widget.ultimateHeight * 0.75)) -
                                widget.ultimateHeight * 0.72,
                            ...
                            child: GestureDetector(
                              onHorizontalDragUpdate:
                                  (DragUpdateDetails update) =>
                                      _onDragUpdateContainer(context, update),
                              onHorizontalDragStart: (DragStartDetails start) =>
                                  _onDragStartContianer(context, start),
                              onHorizontalDragEnd: (DragEndDetails end) =>
                                  _onDragEndContainer(context, end),
                              //  onDoubleTap: ()=>_onDoubleTapContainer(),
                              onDoubleTapDown: _handleDoubleTapDown,
                              onTapUp: (TapUpDetails details) =>
                                  _onTapUp(details),
                              onLongPressEnd:
                                  (LongPressEndDetails longpressend) =>
                                      _onlongPressEnd(longpressend),
                              onLongPressMoveUpdate:
                                  (LongPressMoveUpdateDetails move) =>
                                      _onLongpressUpdate(move),
                              child: Column(
                                children: [
                                  body(),
                                ],
                        ...
                        builder: (context, child) {
                          return Positioned(
                            left: _dragPosition.dx +
                                widget.ultimateWidth *
                                    sequenceAnimation['number_width'].value,
                            top: isDragging
                                ? _dragPosition.dy -
                                    widget.ultimateHeight *
                                        sequenceAnimation['number_height']
                                            .value -
                                    (widgetPosition -
                                        widget.ultimateHeight * 0.74)
                                : !emoji
                                    ? _dragPosition.dy -
                                        widget.ultimateHeight *
                                            sequenceAnimation['number_height']
                            ...
                            child: Opacity(
                              ...
                              child: Text(
                                _dragPercentage.toString().length > 3
                                    ? '${_dragPercentage.toString().substring(0, 3)}'
                                    : _dragPercentage.toString(),
                                style: TextStyle(
                                    fontSize:
                                        sequenceAnimation['font_size'].value,
        ...
        Positioned(bottom: 0, child: Bottom_nav()),
      ],
    );
  }
}
...
class WavePainter extends CustomPainter {
  ...
  _paintAnchor(Canvas canvas, Size size) {
    ...
    canvas.drawCircle(
        Offset(size.width - 20, size.height / 2), 2, circlePainter);
  }

  _paintLine(Canvas canvas, Size size) {
    double x;
    bool checkX; // to check weather the emoji is still in the slider
    if (sliderPosition.dx < widths * 0.11 ||
        sliderPosition.dx > widths * 0.72) {
      checkX = false;
      x = widths * 0.415;
    } else {
      if (isDragging) {
    ...
    path.lineTo(
        sliderPosition.dy - widgetPosiion < heights * 0.76 ||
                sliderPosition.dy - widgetPosiion > heights * 0.80
            ? 20
            : x,
        size.height / 2);
    sliderPosition.dy - widgetPosiion < heights * 0.76 ||
        ...
        ? path.quadraticBezierTo(
            sliderPosition.dx,
            (sliderPosition.dy - widgetPosiion) - heights * 0.77,
            size.width - 20,
            size.height / 2)
        : checkX &&
                isDragging &&
                sliderPosition.dy - widgetPosiion < heights * 0.80
            ? path.cubicTo(sliderPosition.dx - 20, 75, sliderPosition.dx + 15,
                75, sliderPosition.dx + 25, size.height / 2)
            : sliderPosition.dy > heights * 0.78
